...
const { generatePost, generateVisualPrompt, generateNanoBananaImage } = require("./gemini");
...
const IMAGE_COST = 1000000; // 1,000,000 tokens for one image
...
app.post("/generate", async (req, res) => {
  ...
  try {
    ...
    const { content, tokens } = await generatePost({ platform, topic, style });
...
exports.grantTokens = onDocumentCreated({
  ...
  if (subscription.status === "active" || subscription.status === "trialing") {
    ...
    if (!userData.lastSubscriptionId || userData.lastSubscriptionId !== event.params.subscriptionId) {
      console.log(`🎁 Przyznawanie 50M tokenów dla ${userId} (Sub: ${event.params.subscriptionId})`);
      await userRef.set({
        balance: admin.firestore.FieldValue.increment(50000000),
